...
for PLATFORM in $PLATFORMS; do
    ...
    PLATFORM=$PLATFORM sh run-docker.sh /bin/sh -c "cd nextui-music-player/src && make PLATFORM=$PLATFORM" || exit
done
...
for PLATFORM in $PLATFORMS; do
    ...
    adb push -r "dist/Music Player.pak" "/mnt/SDCARD/Tools/$PLATFORM/"
done
