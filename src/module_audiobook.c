...
static void set_toast(const char* message) {
    snprintf(toast_message, sizeof(toast_message), "%s", message);
...
static void cycle_sleep_timer(void) {
    ...
    if (minutes > 0) {
        ...
        set_toast("Sleep timer: end of chapter");
    } else {
        set_toast("Sleep timer off");
    }
}
...
ModuleExitReason AudiobookModule_run(SDL_Surface* screen) {
    Audiobook_init();
    ...
    while (1) {
        ...
        if (!screen_off && !ModuleCommon_isScreenOffHintActive()) {
            int help_state;
            switch (state) {
                case AUDIOBOOK_STATE_CHAPTERS: help_state = AUDIOBOOK_CHAPTERS_HELP_STATE; break;
                case AUDIOBOOK_STATE_SEEKING:
                case AUDIOBOOK_STATE_PLAYING:  help_state = AUDIOBOOK_PLAYING_HELP_STATE; break;
                default:                       help_state = AUDIOBOOK_LIBRARY_HELP_STATE; break;
            }
            ...
            if (PAD_justRepeated(BTN_UP) && count > 0) {
                ...
                if (book) {
                    ...
                    if (Audiobook_loadChapters(book) == 0) {
                        set_toast("No playable chapters");
                        current_key[0] = '\0';
                        current_book_index = -1;
                    } else {
                ...
                if (book) {
                    ...
                    Audiobook_flushProgress();
                    set_toast(book->finished ? "Marked as finished" : "Marked as unfinished");
                }
                dirty = 1;
            }
                ...
                switch (state) {
                    case AUDIOBOOK_STATE_LIBRARY:
                        render_audiobook_library(screen, show_setting, library_selected,
                                                 &library_scroll, toast_message, toast_time);
                        break;
                    ...
                    case AUDIOBOOK_STATE_SEEKING:
                        ...
                        render_toast(screen, "Resuming...", SDL_GetTicks());
                        break;
